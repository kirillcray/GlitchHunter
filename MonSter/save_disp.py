...
def demo(args):
    ...
    img_dir = output_directory / 'img'
    npy_dir = output_directory / 'npy'
    ...
    with torch.no_grad():
        ...
        for (imfile1, imfile2) in tqdm(list(zip(left_images, right_images))):
            ...
            stem = Path(imfile1).stem
            disp = disp.cpu().numpy().squeeze()
            disp = np.round(disp * 256).astype(np.uint16)
            
            img_path = img_dir / f"{stem}.png"
            skimage.io.imsave(str(img_path), disp)

            npy_path = npy_dir / f"{stem}.npy"
            np.save(str(npy_path), disp)


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--restore_ckpt', help="restore checkpoint", default="MonSter/pretrained/mix_all.pth")
    ...
    parser.add_argument('-l', '--left_imgs', help="path to all first (left) frames", default="rectified_images/left/*.png")
    parser.add_argument('-r', '--right_imgs', help="path to all second (right) frames", default="rectified_images/right/*.png")

    parser.add_argument('--output_directory', help="directory to save output", default="depth_maps/")
